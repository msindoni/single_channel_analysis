...
def bc_load_dataframe_asc(file):
    ...
    if len(processed_file[0]) == 5:
          ...
          colnames = ['index','ti','i','tv','v','tp','p', 'tx', 'x']
    ...
    for name,group in grouped:
        voltage_id=group['voltages'].max()
        if voltage_id<=-80:
            voltage_id=group['voltages'].min()
        voltage_id_list+=[voltage_id]*len(group)
    df['voltage_id']=voltage_id_list

    return(df)

def bc_summary_plots(df):
    ...
    fig = make_subplots(rows = int(df['sweep'].max() + 3), cols=1, 
                        subplot_titles=subplot_titles, row_heights = [0.2] * int(df['sweep'].max() + 3))
    ...
    for name, group in df.groupby('sweep'):
            ...
            fig.add_trace(go.Scatter(mode='lines', name=name, x = group['ti'], y=group['i'],  marker=dict(color='black')), row= name + 2, col=1)          
            fig.update_yaxes(title_text= 'Current (pA)', row=name + 2, col=1)
            fig.update_yaxes(range=[-5, 5], row=name + 2, col=1)
